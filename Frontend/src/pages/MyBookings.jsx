...
import { FaTicketAlt, FaMapMarkerAlt, FaCalendarAlt, FaTrash, FaSpinner } from "react-icons/fa";
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans mt-18">
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-12 mt-10 pb-20">
        <div className="space-y-6">
          {bookings.length > 0 ? (
            bookings.map((item) => (
              <div key={item._id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col md:flex-row items-center">
                
                {/* Image - Linked to Tour Data */}
                ...
                <div className="flex-1 p-6 w-full flex flex-col justify-between">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="text-xl font-bold text-slate-800 mb-2">{item.tourId?.title}</h3>
                      <div className="space-y-2 text-slate-400 text-sm">
                        <div className="flex items-center gap-2">
                          <FaMapMarkerAlt size={12} />
                          <span>{item.tourId?.city}, {item.tourId?.country}</span>
                        </div>
                        <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col items-end gap-6">
                      <span className={`text-[10px] px-3 py-1 rounded-full font-bold uppercase tracking-wider ${
                        item.status === 'allowed' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                      }`}>
                        {item.status === 'allowed' ? 'Confirmed' : 'Pending Payment'}
                      </span>
                      <span className="text-xl font-bold text-slate-800">${item.tourId?.price}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-6">
                    <div className="flex gap-3">
                      {item.status === "allowed" ? (
                        <>
                          <button
                            onClick={() => navigate(`/ticket/${item.tourId?._id}`)}
                        ...
                        <div className="bg-amber-50 border border-amber-200 p-3 rounded-xl flex items-start gap-3 max-w-md">
                          <div className="bg-amber-500 text-white w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-bold shrink-0">!</div>
                          <p className="text-[11px] text-amber-800 leading-tight">
                            <strong className="block mb-1">Physical Payment Required</strong>
                            Please visit the agency to pay. Your ticket will unlock here after Admin confirms.
                          </p>
                        </div>
                      )}
                    </div>
